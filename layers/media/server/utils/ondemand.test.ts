...
import { parseVariantName, deriveOnDemand, variantKeyFromPath } from './ondemand'
...
describe('deriveOnDemand (dev miss → derive from the original, registry-independent)', () => {
  it('derives a missing jpeg variant for an existing media and caches it; refuses unresolvable keys', async () => {
    ...
    try {
      ...
      const r = await deriveOnDemand(db, driver, DEFAULT_IMAGE_POLICY, 'a/pic.png-w320.jpeg')
      expect(r?.mime).toBe('image/jpeg')
      expect(await driver.exists!('a/pic.png-w320.jpeg')).toBe(true) // cached to storage

      expect(await deriveOnDemand(db, driver, DEFAULT_IMAGE_POLICY, 'a/pic.png-thumb.jpeg')).toBeNull() // name not parseable
      expect(await deriveOnDemand(db, driver, DEFAULT_IMAGE_POLICY, 'a/missing.png-w320.jpeg')).toBeNull() // no such original
      expect(await deriveOnDemand(db, driver, DEFAULT_IMAGE_POLICY, 'a/pic.png-w320.avif')).toBeNull() // unsupported format
    } finally {
