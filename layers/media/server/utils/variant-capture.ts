...
import { reconcileVariants, type StoredVariant } from './variants'

// Auto-discovery accumulator. `KestrelImg`/`useMediaVariant` stash their concrete specs on the render's
// `event.context.kestrelVariants`; a `beforeResponse` hook (only for publish renders, gated on the
// `x-kestrel-publish` header) drains them into this module Map, keyed by name and unioning formats across
// usages. At the end of a FULL publish / generate `saveDiscoveredVariants` reconciles the union into the
// registry and clears it — so the registry reflects exactly what the last full render actually used.
 ...
 */
export function saveDiscoveredVariants(db: BetterSQLite3Database, opts: { clear?: boolean } = {}): void {
