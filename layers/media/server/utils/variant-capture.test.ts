...
import { recordVariants, collectVariants, clearVariants, saveDiscoveredVariants } from './variant-capture'
...
describe('saveDiscoveredVariants (reconcile into media_settings)', () => {
  ...
  it('writes the discovered set (source:scan) and drains the accumulator', () => {
    ...
    expect(collectVariants()).toEqual([]) // drained
    expect(activeVariants(db, fallback)).toEqual([{ name: 'w320', width: 320, height: null, fit: 'cover', position: 'centre', formats: ['webp', 'jpeg'] }])
  })
  ...
  it('keeps manual/pinned entries and replaces stale scan entries (narrowing)', () => {
    ...
    saveDiscoveredVariants(db)
    const names = activeVariants(db, fallback).map((v) => v.name).sort()
  ...
  it('leaves the registry untouched when nothing was discovered (guards against a capture gap wiping it)', () => {
    ...
    expect(activeVariants(db, fallback).map((v) => v.name)).toEqual(['w320'])
  })
  ...
  it('with { clear: false } keeps the accumulator so repeated calls converge (the generate/prerender topology)', () => {
    ...
    expect(activeVariants(db, fallback).map((v) => v.name).sort()).toEqual(['crop-400x300', 'w320'])
  })
})
