...
import type { ResolvedImagePolicy, VariantFit } from '../../../core/server/utils/kestrel-config'
...
export function resolveVariantRequest(db: BetterSQLite3Database, requestedKey: string): VariantRequest | null {
...
export async function deriveOnDemand(db: BetterSQLite3Database, driver: StorageDriver, policy: ResolvedImagePolicy, requestedKey: string): Promise<{ bytes: Buffer; mime: string } | null> {
