...
describe('listLibrary', () => {
  it('returns immediate child folders and files of a folder, resolved for rendering', () => {
    const r = listLibrary(db, { folder: 'a' }, url)
  ...
  it('tolerates non-numeric page/perPage (NaN) instead of 500ing the listing', () => {
    const r = listLibrary(db, { folder: 'a', page: NaN, perPage: NaN }, url)
  ...
  it('root lists top-level folders and root files (folder stored NULL by the uploader)', () => {
    ...
    const r = listLibrary(db, { folder: '' }, url)
  ...
  it('reports the recursive byte size of each child folder', () => {
    ...
    const a = listLibrary(db, { folder: 'a' }, url)
    expect(a.folders.find((f) => f.path === 'a/b')!.size).toBe(3)
    expect(a.folders.find((f) => f.path === 'a/c')!.size).toBe(0) // empty folder
  })
  it('reports folder existence: the root and registered folders exist, unknown paths do not', () => {
    expect(listLibrary(db, { folder: '' }, url).exists).toBe(true)
    expect(listLibrary(db, { folder: 'a' }, url).exists).toBe(true)
    expect(listLibrary(db, { folder: 'a/b' }, url).exists).toBe(true)
    expect(listLibrary(db, { folder: 'nope' }, url).exists).toBe(false)
    expect(listLibrary(db, { folder: 'a/x' }, url).exists).toBe(false)
  })
  it('sorts files and child folders by the chosen column + direction', () => {
    // folder 'a' files: one.png (size 1), doc.pdf (size 2)
    expect(listLibrary(db, { folder: 'a', sort: 'size' }, url).files.map((f) => f.filename)).toEqual(['one.png', 'doc.pdf'])
    expect(listLibrary(db, { folder: 'a', sort: '-size' }, url).files.map((f) => f.filename)).toEqual(['doc.pdf', 'one.png'])
    expect(listLibrary(db, { folder: 'a', sort: '-name' }, url).files.map((f) => f.filename)).toEqual(['one.png', 'doc.pdf'])
    // child folders: a/b (size 3), a/c (size 0)
    expect(listLibrary(db, { folder: 'a', sort: 'size' }, url).folders.map((f) => f.path)).toEqual(['a/c', 'a/b'])
    expect(listLibrary(db, { folder: 'a', sort: '-size' }, url).folders.map((f) => f.path)).toEqual(['a/b', 'a/c'])
  })
  it('search filters files by filename; type filters to images', () => {
    expect(listLibrary(db, { folder: 'a', search: 'doc' }, url).files.map((f) => f.filename)).toEqual(['doc.pdf'])
    expect(listLibrary(db, { folder: 'a', type: 'image' }, url).files.map((f) => f.filename)).toEqual(['one.png'])
  })
