...
import { previewRelocation, executeRelocation, coerceOpItems, type MediaOp } from './relocate-ops'
...
describe('previewRelocation', () => {
  ...
  it('rejects moving a folder into its own descendant', () => {
    ...
    expect(() => previewRelocation(db, op)).toThrow()
  })
  ...
  it('flags only the conflicting item in a multi-item op (no cross-contamination)', () => {
    ensureFolder(db, 'archive/sub') // archive/sub exists; archive/a.png does NOT
    const op: MediaOp = {
      type: 'move',
  ...
  it('flags an intra-batch collision when two items target the same key (DB has nothing there)', () => {
    ensureFolder(db, 'x'); ensureFolder(db, 'y')
  ...
  it('probes only the op\'s own target keys/paths — never an unconditional full-table scan', () => {
    ...
    previewRelocation(db, { type: 'move', items: [{ type: 'file', id: idOf('pics/a.png') }], dest: 'other' })
...
describe('executeRelocation', () => {
  ...
  it('moves a file (+ derivative) into the destination folder', async () => {
    ...
    const result = await executeRelocation(db, driver, { type: 'move', items: [{ type: 'file', id: a.id }], dest: 'archive' }, 'abort')
  ...
  it('moves a folder recursively (media + folder rows); siblings untouched', async () => {
    ...
    await executeRelocation(db, driver, { type: 'move', items: [{ type: 'folder', path: 'pics/sub' }], dest: 'archive' }, 'abort')
  ...
  it('copies a file: new row + object, source intact', async () => {
    ...
    const result = await executeRelocation(db, driver, { type: 'copy', items: [{ type: 'file', id: a.id }], dest: 'dup' }, 'abort')
  ...
  it('copies a folder recursively: new rows + folder rows, source intact', async () => {
    const b = await put('pics/sub/b.png', 'pics/sub', 'b.png')
    await executeRelocation(db, driver, { type: 'copy', items: [{ type: 'folder', path: 'pics/sub' }], dest: 'archive' }, 'abort')
  ...
  it('renames a file', async () => {
    ...
    const result = await executeRelocation(db, driver, { type: 'rename', items: [{ type: 'file', id: a.id }], name: 'b.pdf' }, 'abort')
  ...
  it('renames a folder recursively', async () => {
    const b = await put('pics/sub/b.png', 'pics/sub', 'b.png')
    await executeRelocation(db, driver, { type: 'rename', items: [{ type: 'folder', path: 'pics/sub' }], name: 'gallery' }, 'abort')
  ...
  it('aborts on a real collision without mutating anything', async () => {
    ...
    await expect(
      executeRelocation(db, driver, { type: 'move', items: [{ type: 'file', id: a.id }], dest: 'archive' }, 'abort'),
    ).rejects.toThrow()
  ...
  it('skips a no-op move (file into its own folder)', async () => {
    const a = await put('pics/a.png', 'pics', 'a.png')
    const result = await executeRelocation(db, driver, { type: 'move', items: [{ type: 'file', id: a.id }], dest: 'pics' }, 'abort')
  ...
  it('rejects moving a folder into its own descendant (guard)', async () => {
    ensureFolder(db, 'pics')
    await expect(
      executeRelocation(db, driver, { type: 'move', items: [{ type: 'folder', path: 'pics' }], dest: 'pics/sub' }, 'abort'),
    ).rejects.toThrow()
  })
  ...
  it('onConflict skip: skips the conflicting item, moves the clean one, occupant untouched', async () => {
    ...
    const result = await executeRelocation(db, driver, {
  ...
  it('onConflict overwrite: a LATER item in the same batch never destroys an EARLIER item that already landed on its target (both survive)', async () => {
    ...
    expect(result[0].status).toBe('moved') // a lands cleanly first
    expect(result[1].status).toBe('skipped') // b refuses to destroy a's row, which just landed on its target
    // a's row is intact at the destination — NOT deleted by b's overwrite
    expect(keyOf(a.id)).toBe('d/a.png')
    expect(await driver.exists!('d/a.png')).toBe(true)
    // b is untouched at its original location
  ...
  it('onConflict overwrite (copy): a LATER item never destroys the fresh row an EARLIER copy just created', async () => {
    ...
    expect(result[1].status).toBe('skipped') // b refuses to destroy the copy of a that just landed
  ...
  it('onConflict overwrite (copy): a genuine pre-existing occupant is still overwritten even when it is an earlier no-op item of the same batch', async () => {
    ...
    expect(keyOf(a.id)).toBeUndefined() // the occupant a really was replaced, as asked
  ...
  it('onConflict rename (folder): keeps the occupant, re-bases the mover under a freed root', async () => {
    ...
    // moved subtree under freed root
  ...
  it('onConflict rename: two movers colliding on the same key get distinct freed names', async () => {
    ...
    // occupant intact
    expect(await driver.exists!('archive/a.png')).toBe(true)
    // both movers landed at distinct freed names
  ...
  it('folder copy: dest dir exists, source dir still present on disk', async () => {
    ...
    // source file still accessible
    expect(keyOf(b.id)).toBe('pics/sub/b.png')
  })

  it('empty folder move: dest dir created, source dir removed from disk', async () => {
    ensureFolder(db, 'empty-src')
    await driver.ensureDir!('empty-src')

    await executeRelocation(db, driver, { type: 'move', items: [{ type: 'folder', path: 'empty-src' }], dest: 'archive' }, 'abort')
  ...
  it('copy + abort onto an occupied target throws at execute (nothing changed)', async () => {
    ...
    await expect(
      executeRelocation(db, driver, { type: 'copy', items: [{ type: 'file', id: a.id }], dest: 'dup' }, 'abort'),
    ).rejects.toThrow()
  ...
  it('rename updates the filename column (not just the storageKey)', async () => {
    ...
    const result = await executeRelocation(db, driver, { type: 'rename', items: [{ type: 'file', id: a.id }], name: 'renamed.png' }, 'abort')
    expect(result[0].status).toBe('renamed')
  ...
  it('move keeps the filename column unchanged', async () => {
    const a = await put('pics/a.png', 'pics', 'a.png')
    await executeRelocation(db, driver, { type: 'move', items: [{ type: 'file', id: a.id }], dest: 'archive' }, 'abort')
    expect(keyOf(a.id)).toBe('archive/a.png')
  ...
  it('copy into the same folder with overwrite is a no-op (keeps exactly one file)', async () => {
    ...
    await executeRelocation(db, driver, { type: 'copy', items: [{ type: 'file', id: a.id }], dest: 'pics' }, 'overwrite')
