...
const calls: Array<{ collection: string; id: number; depth: number; locale: string }> = []
const fakeResolve: ResolveRecord = (collection, id, depth, locale) => {
  calls.push({ collection, id, depth, locale })
  if (id === 999) return null
  return { id, label: `${collection}#${id}`, depthSeen: depth }
}
beforeEach(() => { calls.length = 0 })

const relPop = buildRelationFieldPopulator(fakeResolve)
...
describe('relation field populator (under the field-tree walker)', () => {
  ...
  it('passes the DECREMENTED depth to the related read (the cycle guard)', () => {
    ...
    expect(calls).toEqual([{ collection: 'authors', id: 7, depth: 2, locale: 'de' }])
  })
