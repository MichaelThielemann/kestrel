...
import { desiredSchema, desiredTable } from './desired'
...
import { pages, posts, settings, media, mediaSettings, folders, publishDeps, publishStatus, recordRefs } from '../../../../server/database/schema'
...
describe('desiredSchema — parity with the committed drizzle-kit migrations', () => {
  it('desired schema of the live tables equals the migrated DB → boot auto-sync is a no-op on built-ins', () => {
    const desired = desiredSchema([pages, posts, settings, media, mediaSettings, folders, publishDeps, publishStatus, recordRefs])
