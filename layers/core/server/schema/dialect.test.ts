...
import { sqlite, postgres, resolveDialect, type Dialect } from './dialect'
import { renderSqlite } from './render-sqlite'
import { introspect } from './introspect'
import { diffSchema } from './diff'
import { syncSchema, planOps } from './sync'
...
describe('sqlite dialect', () => {
  ...
  it('introspect delegates to the SQLite introspector (same snapshot)', () => {
    ...
    expect(sqlite.introspect(db)).toEqual(introspect(db))
    db.close()
  })
...
describe('postgres slot (reserved, fail-loud)', () => {
  ...
  it('introspect throws a clear not-implemented error', () => {
    const db = new Database(':memory:')
    expect(() => postgres.introspect(db)).toThrow(/postgres/i)
    db.close()
  })
...
describe('sync threads the dialect (the seam)', () => {
  it('drives introspect + render through the injected dialect, producing identical output', () => {
    ...
    const { applied } = syncSchema(db, desired, {}, spy)
    ...
    expect(syncSchema(fresh, desired).applied).toEqual(applied)
  ...
  it('planOps reads the live schema through the injected dialect', () => {
    ...
    expect(planOps(db, desiredSchema([things]), spy).length).toBeGreaterThan(0)
  ...
  it('the postgres slot makes syncSchema fail loud rather than silently emit SQLite DDL', () => {
    const db = new Database(':memory:')
    expect(() => syncSchema(db, desiredSchema([things]), {}, postgres)).toThrow(/postgres/i)
    db.close()
  })
})
