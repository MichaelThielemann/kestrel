import { fieldIs, isSingleRefColumn, type CollectionDef, type Condition, type FieldDef, type FieldType, type Localized } from './defineCollection'
import { resolveFieldLayout, type LayoutNode } from './field-layout'
...
export function serializeCollection(def: CollectionDef): SerializedCollection {
  return {
    name: def.name,
    mode: def.mode,
...
// Structurally typed (not importing BlockDef) so `core` stays independent of the `fields` layer.
// Block fields serialize identically to collection fields, so the admin FieldRenderer reuses them as-is.
