...
import { create, list, getOne, update, remove, resolveTranslations, parseFilter, putSingleton, isUniqueViolation } from './crud'
import { withReadCapture } from './read-capture'
...
beforeEach(() => {
  clearBlocks()
  registerBlock(defineBlock({ name: 'hero', fields: {} }))
  db = createTestDb()
})
...
describe('crud — collections', () => {
  ...
  it('rejects a duplicate locale within one translation group (409)', () => {
    ...
    expect(() => create(db, pagesCollection, { title: 'Home2', translationGroup: en.translationGroup as string }))
      .toThrowError(/Conflict.*duplicate|409/)
  })
  ...
  it('updates and deletes one variant', () => {
    ...
    expect(() => getOne(db, pagesCollection, en.id as number)).toThrowError(/not found|404/)
  })
  ...
  it('optimistic concurrency: a stale expectedUpdatedAt is rejected with 409, a matching one saves', () => {
    ...
    expect(() => update(db, pagesCollection, en.id as number, { title: 'B' }, { expectedUpdatedAt: stored - 5000 }))
      .toThrowError(/409|changed since|conflict/i)
    expect((getOne(db, pagesCollection, en.id as number) as Record<string, unknown>).title).toBe('Home') // untouched
  ...
  it('pageLike update normalizes an explicit slug, and leaves the slug alone when path is omitted', () => {
    ...
    expect((update(db, pagesCollection, r.id as number, { title: 'X2' }) as Record<string, unknown>).path).toBe('/y') // path untouched
  })
  ...
  it('maps a UNIQUE violation on update (locale collision) to 409', () => {
    ...
    expect(() => update(db, pagesCollection, de.id as number, { locale: 'en' })).toThrowError(/Conflict.*duplicate|409/)
  })
  ...
  it('coerces array-valued locale/sort query params instead of throwing', () => {
    create(db, pagesCollection, { title: 'A' })            // en
    create(db, pagesCollection, { title: 'B', locale: 'de' })
...
describe('crud — write-transforms recurse into repeater entries + block props (L7)', () => {
  it('derives a nested slug from its sibling (inside a repeater and inside block props)', () => {
    registerBlock(defineBlock({ name: 'card', fields: { title: { type: 'text' }, slug: { type: 'slug', options: { from: 'title' } } } }))
    ...
    for (const stmt of renderSqlite(diffSchema(desiredSchema([c.table]), {}))) sqlite.exec(stmt)
...
describe('crud — multi-field default applies on create when omitted (L6)', () => {
  ...
  it('an API create that omits a multi-choice / json field gets its declared default from the column', () => {
    const sqlite = new Database(':memory:')
    for (const stmt of renderSqlite(diffSchema(desiredSchema([items.table]), {}))) sqlite.exec(stmt)
    const idb = drizzle(sqlite)
...
describe('crud — list() filter type coercion', () => {
  ...
  beforeEach(() => {
    const sqlite = new Database(':memory:')
    for (const stmt of renderSqlite(diffSchema(desiredSchema([flags.table]), {}))) sqlite.exec(stmt)
    fdb = drizzle(sqlite)
  })
...
describe('crud — pageLike global slug uniqueness (registry-backed)', () => {
  ...
  beforeEach(() => {
    ...
    const sqlite = new Database(':memory:')
    for (const stmt of renderSqlite(diffSchema(desiredSchema([pagesC.table, postsC.table]), {}))) sqlite.exec(stmt)
    rdb = drizzle(sqlite)
  })
  ...
  it('rejects an explicit slug that collides across collections (409)', () => {
    ...
    expect(() => create(rdb, postsC, { title: 'Y', path: '/x' })).toThrowError(/already in use|409/)
  })
  ...
  it('rejects a locale-only PATCH that re-routes a row into a cross-collection collision (409)', () => {
    ...
    expect(() => update(rdb, pagesC, a.id as number, { locale: 'de' })).toThrowError(/already in use|409/)
  })
})
...
describe('crud — conditional required fields', () => {
  ...
  it('putSingleton() re-enforces conditional-required on the merged record', () => {
    ...
    expect(() => putSingleton(db, banner, undefined, { format: 'image' })).toThrowError(/Validation failed|required|400/)
    expect((putSingleton(db, banner, undefined, { format: 'image', caption: 'Hi' }) as Record<string, unknown>).caption).toBe('Hi')
  })
})
...
describe('publishedOnly read scope', () => {
  ...
  it('getOne with publishedOnly 404s a draft but returns a published row', () => {
    ...
    expect(() => getOne(db, pagesWithStatus, draft.id, 0, 'en', true)).toThrow(/not found/)
...
describe('crud — per-row translation status (list aggregation)', () => {
  // Counts top-level db.select() calls so we can assert there is no per-row N+1: a page of N rows
  // must issue a constant number of selects regardless of N.
  function withSelectCounter(database: ReturnType<typeof createTestDb>) {
    ...
    return { db: proxy as typeof database, selects: () => selects }
  }
