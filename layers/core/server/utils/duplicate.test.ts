...
import { duplicateRecord, withCopySuffix } from './duplicate'
...
import { clearWriteListeners, registerWriteListener } from './write-events'
import { maintainRecordRefs, findReferrers } from './record-ref-index'
...
import mediaCollection from '../../../media/server/collections/media'
...
beforeEach(() => {
  ...
  db = createTestDb()
  for (const stmt of renderSqlite(diffSchema(desiredSchema([things.table, gal.table, badges.table, uposts.table, capped.table, boards.table]), {}))) {
    db.run(sql.raw(stmt))
  }
  clearRegistry()
  ...
  clearWriteListeners()
  registerWriteListener((event) => maintainRecordRefs(db, event)) // mirrors plugins/03.record-refs.ts
})
afterEach(() => {
  clearBlocks()
  clearWriteListeners()
...
describe('duplicateRecord — references are SHARED, never deep-copied, and reindexed', () => {
  it('copies relation/media/link FKs by value and rebuilds record_refs for the new id', () => {
    ...
    // emitWrite → maintainRecordRefs rebuilt the forward edges for the NEW row (no extra work in duplicate).
    expect(findReferrers(db, 'media', 7)).toContainEqual({ collection: 'things', id: copyId })
    expect(findReferrers(db, 'things', 1)).toContainEqual({ collection: 'things', id: copyId })
    expect(findReferrers(db, 'things', 3)).toContainEqual({ collection: 'things', id: copyId })
  })
})
...
describe('duplicateRecord — translatable (Option A: lone row in a new group)', () => {
  ...
  it('keeping the source group would violate UNIQUE(group, locale) → 409 (why Option A is forced)', () => {
    ...
    expectStatus(
      () => create(db, pagesCollection, { title: 'Grp2', path: '/grp2', locale: src.locale, translationGroup: src.translationGroup }),
      409,
    )
  })
})
