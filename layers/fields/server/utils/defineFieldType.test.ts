...
import { defineFieldType, constrain, opt, optArr } from './defineFieldType'
import { getFieldType, fieldTypes } from '../field-registry/index'
...
describe('defineFieldType — consumer-defined field types, end to end', () => {
  ...
  it('forwards a consumer-supplied transform (must not be silently dropped)', () => {
    ...
    defineFieldType({ name: 'color', column: (n, f) => constrain(text(n), f), validator: (f) => opt(z.string(), f), transform } as never)
