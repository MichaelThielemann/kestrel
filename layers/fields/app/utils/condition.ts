import type { Condition, ConditionOperator, ConditionRule } from '../../../core/server/utils/defineCollection'
...
// v1 path scope is SIBLING-ONLY. A bare name or `./name` resolves a sibling in the current scope;
// cross-scope forms (`/abs`, `../parent`, dotted `a.b`) are out of v1 and resolve to undefined — so the
// rule is simply false (the field hides), keeping the evaluator total: it never throws on a bad path.
