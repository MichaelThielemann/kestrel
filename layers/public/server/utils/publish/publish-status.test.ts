...
const rowFor = (db: ReturnType<typeof drizzle>, route: string) =>
...
describe('recordPublishStatus / clearPublishStatus', () => {
  ...
  it('stores updatedAt as UNIX seconds (a Date, not raw milliseconds — the #6-bug guard)', () => {
  ...
  it('degrades gracefully when the publish_status table is absent (a not-yet-migrated deploy)', () => {
    const bare = drizzle(new Database(':memory:')) // no publish_status table
