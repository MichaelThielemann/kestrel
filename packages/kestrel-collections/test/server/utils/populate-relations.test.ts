...
describe('skipMissing — only getOne 404 (stale/deleted/draft) becomes null; other faults propagate (fail-loud)', () => {
  ...
  it('maps the real pipeline failure — a tagged NotFound, what readOne actually throws — to null', () => {
    expect(skipMissing(() => { throw new NotFound({ collection: 'authors', id: 9999 }) })).toBeNull()
  })

  it('RE-THROWS an h3-shaped error with a statusCode but no _tag (no longer a recognized "missing" signal)', () => {
    expect(() => skipMissing(() => { throw { statusCode: 404, statusMessage: 'not found' } })).toThrow()
  })

  it('RE-THROWS a KestrelError that is not NotFound (e.g. Forbidden) instead of dropping the relation silently', () => {
    expect(() => skipMissing(() => { throw { _tag: 'Forbidden', reason: 'no' } })).toThrow()
  })
