 ...
 * the client bundle. Mirrors `@michaelthielemann/kestrel-core/client`'s reason for existing (see that entry's own TSDoc) —
 * built after `@michaelthielemann/kestrel-core` learned the hard way that a server-only import reaching the browser bundle
...
// harmless: they live at separate entry points (`@michaelthielemann/kestrel-fields` vs `@michaelthielemann/kestrel-fields/client`) and are
