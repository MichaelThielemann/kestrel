...
import { resolveFieldLayout, validateFieldLayoutsDeep } from '../../../src/server/utils/field-layout.js'
import type { CollectionDef } from '../../../src/server/utils/defineCollection.js'
import type { LayoutNode, LayoutRow } from '../../../src/server/utils/field-layout.js'

// `tracks` only exists on the 'row' arm of `LayoutNode`; these cases always resolve to a row.
const tracksOf = (node: LayoutNode): LayoutRow['tracks'] => (node as LayoutRow).tracks
...
describe('resolveFieldLayout', () => {
  ...
  it('flex weights: a bare number becomes an fr track', () => {
    ...
    expect(tracksOf(resolveFieldLayout([['a|1.5', 'b']], ['a', 'b'], 'ctx')[0]!)).toEqual([1.5, 1])
  })

  it('CSS lengths/percents pass through, other tracks default to 1fr', () => {
    expect(tracksOf(resolveFieldLayout([['a|30%', 'b']], ['a', 'b'], 'ctx')[0]!)).toEqual(['30%', 1])
    expect(tracksOf(resolveFieldLayout([['a|12rem', 'b|200px']], ['a', 'b'], 'ctx')[0]!)).toEqual(['12rem', '200px'])
    expect(tracksOf(resolveFieldLayout([['a|1fr', 'b|2em']], ['a', 'b'], 'ctx')[0]!)).toEqual([1, '2em'])
  })
  ...
  it('rejects a zero (or negative) weight/length — a 0fr track would collapse the field to a silent sliver', () => {
    ...
    expect(tracksOf(resolveFieldLayout([['a|0.5', 'b']], ['a', 'b'], 'ctx')[0]!)).toEqual([0.5, 1])
    expect(tracksOf(resolveFieldLayout([['a|0.5%', 'b']], ['a', 'b'], 'ctx')[0]!)).toEqual(['0.5%', 1])
  })
