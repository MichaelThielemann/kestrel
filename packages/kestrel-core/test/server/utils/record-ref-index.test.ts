...
import { maintainRecordRefs, deadTargets, collectionMayReference, deriveLocatedDeadRefs, recordDeadRefs, findReferrers, findReferrersForMany, findBrokenRefs } from '../../../src/server/utils/record-ref-index.js'
import { list } from '../../../src/server/utils/crud.js'
...
import { defineCollection, type FieldDef } from '../../../src/index.js'
...
// This suite exercises `record-ref-index.ts`'s functions directly against a raw, unwrapped drizzle
// instance (unchecked — no `<Module>Db` ownership adapter, on purpose: it's the pure-logic unit suite).
// `DB`/`WriteDB`/`RebuildDB` are branded so a raw handle no longer satisfies them structurally —
// this cast is the one place that deliberately steps around that, matching how `makeModuleDb`'s own
// construction site vouches for the brand.
function asContentDb(db: BetterSQLite3Database): ContentDb {
...
describe('maintainRecordRefs', () => {
  it('inserts the extracted edges on create (single relation via `${name}Id`, media)', () => {
    const db = freshDb()
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 1, authorId: 5, coverId: 9 } })
  ...
  it('replaces a source\'s edges on update — old gone, new in', () => {
    const db = freshDb()
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 1, authorId: 5, coverId: 9 } })
    maintainRecordRefs(asContentDb(db), { def: pages, before: { id: 1, authorId: 5, coverId: 9 }, after: { id: 1, authorId: 8, coverId: null } })
    expect(edges(db)).toEqual(['pages:1→users:8'])
  })

  it('removes the source\'s edges on delete', () => {
    const db = freshDb()
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 1, authorId: 5 } })
    maintainRecordRefs(asContentDb(db), { def: pages, before: { id: 1, authorId: 5 }, after: null })
  ...
  it('only touches the written source — other records\' edges survive a re-save', () => {
    const db = freshDb()
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 1, authorId: 5 } })
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 2, authorId: 6 } })
    maintainRecordRefs(asContentDb(db), { def: pages, before: { id: 1, authorId: 5 }, after: { id: 1, authorId: 7 } })
  ...
  it('scopes edges by source collection (same id in two collections does not collide)', () => {
    ...
    maintainRecordRefs(asContentDb(db), { def: pages, before: null, after: { id: 1, authorId: 5 } })
    maintainRecordRefs(asContentDb(db), { def: posts, before: null, after: { id: 1, authorId: 6 } })
  ...
  it('records nothing when a record has no references', () => {
    ...
    maintainRecordRefs(asContentDb(db), { def: plain, before: null, after: { id: 1, title: 'x' } })
    expect(edges(db)).toEqual([])
  })
  ...
  it('indexes edges nested in block content', () => {
    ...
    maintainRecordRefs(asContentDb(db), {
...
describe('deadTargets — batched dead-ness classification', () => {
  ...
  it('flags missing rows, unpublished (status-bearing) rows, and unknown collections; spares alive ones', () => {
    ...
    const dead = deadTargets(asContentDb(db), [
...
describe('deriveLocatedDeadRefs / recordDeadRefs', () => {
  ...
  it('returns only the stale refs, located, with a reason', () => {
    ...
    expect(deriveLocatedDeadRefs(asContentDb(db), pagesC.def, { id: 99, authorId: 11 })).toEqual([
      { field: 'author', collection: 'users', id: 11, reason: 'missing' },
    ])
    expect(deriveLocatedDeadRefs(asContentDb(db), pagesC.def, { id: 99, authorId: 10 })).toEqual([])
  })
  ...
  it('recordDeadRefs loads the row by id then derives (empty for a missing row)', () => {
    ...
    expect(recordDeadRefs(asContentDb(db), pagesC, 50)).toEqual([{ field: 'author', collection: 'users', id: 11, reason: 'missing' }])
    expect(recordDeadRefs(asContentDb(db), pagesC, 9999)).toEqual([])
  })
})
...
describe('list() — $hasDeadRefs sidecar', () => {
  ...
  it('marks rows whose references point at a dead target, leaving healthy rows false', () => {
    ...
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 100, authorId: 10 } })
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 101, authorId: 11 } })
...
describe('findReferrers / findBrokenRefs — reverse lookups', () => {
  ...
  it('findReferrers lists the distinct records that point at a target', () => {
    const db = setupTargets()
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 100, authorId: 10 } })
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 101, authorId: 10 } })
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 102, authorId: 99 } }) // points elsewhere
    expect(findReferrers(asContentDb(db), 'users', 10)!.map((r) => `${r.collection}:${r.id}`).sort()).toEqual(['pages:100', 'pages:101'])
    expect(findReferrers(asContentDb(db), 'users', 77)).toEqual([]) // nobody links here
  })
  ...
  it('findBrokenRefs returns only edges whose target is dead, with the reason (missing + unpublished)', () => {
    ...
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 100, authorId: 11 } }) // → users:11 (missing)
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 101, authorId: 10 } }) // → users:10 (alive)
    maintainRecordRefs(asContentDb(db), { def: linkerDef, before: null, after: { id: 5, refId: 2 } }) // → pages:2 (draft → unpublished)
    maintainRecordRefs(asContentDb(db), { def: linkerDef, before: null, after: { id: 6, refId: 1 } }) // → pages:1 (published → alive)
    const broken = findBrokenRefs(asContentDb(db))!
  ...
  it('findBrokenRefs is empty when every target is alive', () => {
    const db = setupTargets()
    maintainRecordRefs(asContentDb(db), { def: pagesC.def, before: null, after: { id: 100, authorId: 10 } }) // users:10 alive
    expect(findBrokenRefs(asContentDb(db))).toEqual([])
  })
})
...
describe('findReferrersForMany — batched referrer counts for a selection', () => {
  ...
  it('counts distinct external referrers per target, omitting targets nobody links to', () => {
    ...
    expect(findReferrersForMany(asContentDb(db), 'users', [10, 11])).toEqual({ 10: 2 })
    expect(findReferrersForMany(asContentDb(db), 'users', [11])).toEqual({})
  })
  ...
  it('excludes a referrer that is itself in the deletion selection (deleting a linked pair → no false warning)', () => {
    ...
    maintainRecordRefs(asContentDb(db), { def: nodes, before: null, after: { id: 1, parentId: 2 } }) // node 1 → node 2
    maintainRecordRefs(asContentDb(db), { def: nodes, before: null, after: { id: 3, parentId: 2 } }) // node 3 → node 2
    // Deleting {2,3}: node 3 is in the selection, so it is not counted as an external referrer of node 2.
    expect(findReferrersForMany(asContentDb(db), 'nodes', [2, 3])).toEqual({ 2: 1 }) // only node 1 remains
    // Deleting just {2}: both node 1 and node 3 are external referrers.
    expect(findReferrersForMany(asContentDb(db), 'nodes', [2])).toEqual({ 2: 2 })
  })

  it('returns {} for an empty selection', () => {
    expect(findReferrersForMany(asContentDb(freshDb()), 'pages', [])).toEqual({})
  })
  ...
  it('returns null (indeterminate, not "no referrers") for a not-yet-migrated index (bare DB)', () => {
    ...
    expect(findReferrersForMany(asContentDb(bare), 'pages', [1, 2, 3])).toBeNull()
  })
})
...
describe('findReferrers / findReferrersForMany / findBrokenRefs — indeterminate vs. genuinely empty', () => {
  it('a missing record_refs table is null (could not check), not [] (checked, no referrers)', () => {
    ...
    expect(findReferrers(asContentDb(bare), 'pages', 1)).toBeNull()
    expect(findBrokenRefs(asContentDb(bare))).toBeNull()
  })
})
