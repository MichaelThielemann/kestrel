...
import { resolveKestrel, parseLocaleList, DEFAULT_IMAGE_POLICY, resolveS3Settings, layerKestrelOption, normalizeExemptPrefix } from '../../../src/server/utils/kestrel-config.js'
...
describe('resolveS3Settings — shared <PREFIX>_* env → config → default resolver', () => {
  ...
  it('reads the four non-secret fields under the given env prefix, env winning over config', () => {
    ...
    expect(resolveS3Settings({ bucket: 'cfg' }, {}, 'KESTREL_OUTPUT_S3').bucket).toBe('cfg')
...
describe('resolveKestrel — precedence is KESTREL_* env → config → default (env is the override escape-hatch)', () => {
  ...
  it('locales + primaryLocale (env overrides config)', () => {
    ...
    expect(resolveKestrel({ locales: ['fr'] }, { KESTREL_LOCALES: 'en,de' }, root).supportedLocales).toEqual(['en', 'de'])
    ...
    expect(resolveKestrel({ locales: ['en', 'de'], primaryLocale: 'de' }, { KESTREL_PRIMARY_LOCALE: 'en' }, root).primaryLocale).toBe('en')
  ...
  it('prefixPrimary: KESTREL_PREFIX_PRIMARY_LOCALE → config → default false (env boolean wins)', () => {
    ...
    expect(resolveKestrel({ prefixPrimaryLocale: true }, { KESTREL_PREFIX_PRIMARY_LOCALE: 'false' }, root).prefixPrimary).toBe(false)
  ...
  it('siteUrl (raw, not normalised): env → config → empty', () => {
    ...
    expect(resolveKestrel({ siteUrl: 'https://cfg.io' }, { KESTREL_SITE_URL: 'https://env.io' }, root).siteUrl).toBe('https://env.io')
  ...
  it('siteName / siteDescription (for llms.txt): env → config → empty', () => {
    ...
    expect(resolveKestrel({ siteName: 'Cfg' }, { KESTREL_SITE_NAME: 'Env' }, root).siteName).toBe('Env')
  ...
  it('output (static-publish target): dir/publicDir/driver/auto/reconcile with KESTREL_OUTPUT_* → config → default', () => {
    ...
    expect(resolveKestrel({ output: { verbose: true } }, {}, root).output.verbose).toBe(true)
  ...
  it('media.s3: non-secret block follows KESTREL_S3_* → config → default; secrets stay out', () => {
    ...
    expect(resolveKestrel({ media: { s3: { bucket: 'cfg' } } }, {}, root).media.s3.bucket).toBe('cfg')
  ...
  it('imagePolicy.presets: only the config-authored named variants (never the desugared width ladder)', () => {
    ...
    expect(resolveKestrel({}, {}, root).media.imagePolicy.presets).toEqual([])
  })
})

describe('deliveryExempt — extra path prefixes the live catch-all must fall through on', () => {
  it('normalizes a bare segment, a leading-slash-only, and an already-normal entry to the same slash-terminated form', () => {
    expect(normalizeExemptPrefix('health')).toBe('/health/')
    expect(normalizeExemptPrefix('/health')).toBe('/health/')
    expect(normalizeExemptPrefix('/health/')).toBe('/health/')
    expect(normalizeExemptPrefix('  /feed.xml  ')).toBe('/feed.xml/')
    expect(normalizeExemptPrefix('   ')).toBe('') // blank in, blank out — filtered by the caller
  })

  it('resolveKestrel normalizes + dedupes config entries, and empty by default', () => {
    expect(resolveKestrel({}, {}, root).deliveryExempt).toEqual([])
    expect(resolveKestrel({ deliveryExempt: ['health', '/feed.xml', 'health'] }, {}, root).deliveryExempt)
      .toEqual(['/health/', '/feed.xml/'])
  })

  it('KESTREL_DELIVERY_EXEMPT (comma-separated) overrides the config array wholesale', () => {
    expect(resolveKestrel({ deliveryExempt: ['/config-only/'] }, { KESTREL_DELIVERY_EXEMPT: 'health, /feed.xml' }, root).deliveryExempt)
      .toEqual(['/health/', '/feed.xml/'])
  })
})
