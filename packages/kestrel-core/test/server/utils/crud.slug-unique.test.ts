...
import { create, update } from '../../../src/server/utils/crud.js'
...
// A ValidationFailed (400 at the HTTP edge) whose `issues` carry the field-keyed message the editor maps
// onto the slug widget.
const expectSlugConflict = (fn: () => unknown) => {
  try {
    fn()
  } catch (e) {
    const err = e as { _tag?: string, issues?: { path: (string | number)[], message: string }[] }
    expect(err._tag).toBe('ValidationFailed')
    expect(err.issues?.[0]?.path).toEqual(['slug'])
    expect(err.issues?.[0]?.message).toMatch(/already exists/i)
    return
  }
...
beforeEach(() => {
  db = createTestDb()
  for (const stmt of renderSqlite(diffSchema(desiredSchema([galleries.table, revisionsTable('gal')]), {}))) db.run(sql.raw(stmt))
})
