...
import { serializeCollection, serializeField, serializeBlock, type SerializedField } from '../../../src/server/utils/serialize-collection.js'
...
describe('serializeCollection', () => {
  ...
  it('defaults actions to an empty list, and otherwise passes the caller-supplied list through verbatim', () => {
    expect(serializeCollection({ name: 'x', mode: 'multi', translatable: false, fields: {} }).actions).toEqual([])
    const actions = [{ name: 'archive', route: { url: '/api/posts/archive', method: 'POST' as const }, kind: 'bulk' as const }]
    expect(serializeCollection({ name: 'posts', mode: 'multi', translatable: false, fields: {} }, actions).actions).toBe(actions)
  })
})
