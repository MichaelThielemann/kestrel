...
import { fieldIs, isHardRequired, regenerateBlockIds, resolveColumnName } from '@michaelthielemann/kestrel-core'
import type { BuiltCollection, FieldOf } from '@michaelthielemann/kestrel-core'
import { slugSourceKey, dedupeSourcePath } from './page-slug.js'
import { primaryLocale, prefixPrimaryLocale } from './locale.js'
import { allCollections } from './registry.js'
...
/**
 * The insert body for a NEW draft copy of one record. It is fed to the `createOne` chain, so validation,
 * the fresh translationGroup nanoid, the slug de-dup, field transforms, the unique-slug check, the insert
 * AND its after-steps (`reindexRefs` rebuilds `record_refs`, `planPublish` drives the publish queue) all
 * come for free — there is NO parallel insert path. The copy:
 ...
 * @public
 */
export function buildDuplicateBody(db: DB, c: BuiltCollection, id: number): Row {
