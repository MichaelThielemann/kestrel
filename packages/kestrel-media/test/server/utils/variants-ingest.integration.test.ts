...
function asMediaDb(db: BetterSQLite3Database): MediaDb {
...
async function manifestKeys(db: BetterSQLite3Database): Promise<string[]> {
  ...
  const derived = await deriveImage(await makePng(800, 600), { ...policy, variants: activeVariants(asMediaDb(db), policy.variants) })
