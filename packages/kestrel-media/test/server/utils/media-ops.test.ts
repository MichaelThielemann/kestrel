...
import { collectAffected, bulkUsages, deleteAffected, previewDelete } from '../../../src/server/utils/media-ops.js'
import { ensureFolder } from '../../../src/server/utils/folders.js'
import { clearRegistry, create, defineCollection, ensureRevisionsTable, registerCollection, sqliteClientOf, buildCollection, createLocalDriver   } from '@michaelthielemann/kestrel-core'
...
describe('collectAffected', () => {
  it('resolves a file item to its single media row', () => {
    const r = collectAffected(asMediaDb(db), [{ type: 'file', id: idOf('pics/a.png') }])
  ...
  it('expands a folder item to itself + descendants (media + folder rows), not sibling-prefixes', () => {
    const r = collectAffected(asMediaDb(db), [{ type: 'folder', path: 'pics' }])
  ...
  it('returns folders deepest-first and dedupes overlapping items', () => {
    const r = collectAffected(asMediaDb(db), [{ type: 'folder', path: 'pics' }, { type: 'folder', path: 'pics/sub' }])
  ...
  it('never reads the folders table for an all-file item list (the list would go unused)', () => {
    ...
    collectAffected(asMediaDb(db), [{ type: 'file', id: idOf('pics/a.png') }, { type: 'file', id: idOf('other/d.png') }])
...
describe('deleteAffected', () => {
  ...
  it('previewDelete reports the impact + usages without mutating', async () => {
    ensureFolder(asMediaDb(db), 'pics/sub')
    ...
    const report = previewDelete(asMediaDb(db), db, [{ type: 'folder', path: 'pics' }])
  ...
  it('execute deletes media rows + storage objects + folder rows (cascade); siblings survive', async () => {
    ensureFolder(asMediaDb(db), 'pics/sub'); ensureFolder(asMediaDb(db), 'pics-archive')
    seed('pics/a.png', 'pics', 'a.png'); await driver.put('pics/a.png', Buffer.from('x'), 'image/png')
    seed('pics/sub/b.png', 'pics/sub', 'b.png'); await driver.put('pics/sub/b.png', Buffer.from('y'), 'image/png')
    seed('pics-archive/c.png', 'pics-archive', 'c.png')
    await deleteAffected(asMediaDb(db), driver, [{ type: 'folder', path: 'pics' }])
  ...
  it('never wipes unmanaged objects under the folder (e.g. an extension blob namespace) on delete', async () => {
    ensureFolder(asMediaDb(db), 'galleries-secure')
    ...
    await deleteAffected(asMediaDb(db), driver, [{ type: 'folder', path: 'galleries-secure' }])
    expect(await driver.exists!('galleries-secure/cover.png')).toBe(false)
    expect(await driver.exists!('galleries-secure/g1/secret.bin')).toBe(true)
  })
  it('file-only delete does not remove any directory', async () => {
    ensureFolder(asMediaDb(db), 'pics')
    const a = seed('pics/a.png', 'pics', 'a.png'); await driver.put('pics/a.png', Buffer.from('x'), 'image/png')
    await deleteAffected(asMediaDb(db), driver, [{ type: 'file', id: a.id }])
    expect(existsSync(join(uploadsDir, 'pics'))).toBe(true)
  })
  it('a storage delete failure for one item is best-effort: later items still get deleted and the failure is reported, not thrown', async () => {
    ...
    const report = await deleteAffected(asMediaDb(db), driver, [{ type: 'folder', path: 'pics' }])
