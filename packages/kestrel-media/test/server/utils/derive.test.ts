...
describe('deriveImage', () => {
  ...
  it('degrades a variant with a non-positive-integer height to a PROPORTIONAL resize (never crashes sharp)', async () => {
    ...
    // it degrades to a width-only resize.
    const policy = {
      ...DEFAULT_IMAGE_POLICY,
      variants: [
        { name: 'w320', width: 320, height: '' as unknown as null, fit: 'cover' as const, position: 'centre', formats: ['webp'] as ('webp' | 'jpeg')[] },
        { name: 'w640', width: 640, height: 0 as unknown as null, fit: 'cover' as const, position: 'centre', formats: ['webp'] as ('webp' | 'jpeg')[] },
      ],
    }
    const r = await deriveImage(await makePng(800, 600), policy)
    expect(r.variants.map((v) => v.name).sort()).toEqual(['w320', 'w640'])
