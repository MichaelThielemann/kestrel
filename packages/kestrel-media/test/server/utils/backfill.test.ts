...
import { createLocalDriver, DEFAULT_IMAGE_POLICY, clearPipelines, eventsOf, registerAfterStep  } from '@michaelthielemann/kestrel-core'
import type { StorageDriver, ResolvedVariant  } from '@michaelthielemann/kestrel-core'
import { planBackfill, backfillRow, runBackfill } from '../../../src/server/utils/backfill.js'
...
/** Probes `emitMediaWrite`'s after-step-only bypass path with a plain observer after-step. */
const probeEvents = (events: unknown[]) => {
  clearPipelines()
...
describe('backfillRow (integration: get → derive missing → put → update → prune-last)', () => {
  it('generates the missing variants, prunes the orphan object, and rewrites the manifest', async () => {
    ...
    try {
      ...
      const r = await backfillRow(asMediaDb(db), driver, { id: 1, storageKey: 'a/pic.png', mime: 'image/png', width: 800, derivatives: manifest }, specs, DEFAULT_IMAGE_POLICY)
  ...
  it('emits a media write event so pages embedding the pruned derivative re-render', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'kestrel-backfill-'))
    const raw: { before: unknown; after: unknown; def: { name: string } }[] = []
    probeEvents(raw)
    try {
      const db = createTestDb()
      const driver = createLocalDriver({ dir, baseUrl: '/uploads' })
      ...
      await backfillRow(asMediaDb(db), driver, { id: 1, storageKey: 'a/pic.png', mime: 'image/png', width: 800, derivatives: manifest }, [spec('w320', 320, null, ['webp'])], DEFAULT_IMAGE_POLICY)

      expect(raw.map((e) => ({ name: e.def.name, before: e.before, after: e.after })))
        .toEqual([{ name: 'media', before: { id: 1 }, after: { id: 1 } }])
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
  ...
  it('emits nothing for a row that is already reconciled', async () => {
    ...
    probeEvents(events)
    const db = createTestDb()
    const r = await backfillRow(asMediaDb(db), createLocalDriver({ dir: tmpdir(), baseUrl: '/uploads' }), { id: 1, storageKey: 'a/pic.png', mime: 'image/png', width: 800, derivatives: {} }, [], DEFAULT_IMAGE_POLICY)
...
describe('runBackfill', () => {
  it('check mode reports the plan across raster rows without writing (skips non-raster)', async () => {
    ...
    try {
      ...
      const report = await runBackfill(asMediaDb(db), createLocalDriver({ dir, baseUrl: '/uploads' }), DEFAULT_IMAGE_POLICY, { check: true })
  ...
  async function runSeeded(seed: (db: Db, driver: StorageDriver) => Promise<void> | void) {
    ...
    try {
      ...
      const report = await runBackfill(asMediaDb(db), driver, DEFAULT_IMAGE_POLICY)
