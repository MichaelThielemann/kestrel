...
import { useMediaDb, type MediaDb } from '../db/media-db.js'
import type { AnySQLiteTable } from 'drizzle-orm/sqlite-core'
import { media } from '../collections/media.js'
import { folders } from '../database/folders.js'
import { isUnder, parentOf, childName, rewritePrefix } from './folder-paths.js'
import { sanitizeFolder, buildKey, suggestFreeName } from './naming.js'
import { planRelocateInto, planRename, type ItemPlan } from './relocate-plan.js'
import { relocateMedia, duplicateMedia } from './storage-relocate.js'
...
/** A bulk relocation request: the items, its type, and the destination/new name it targets.
 * @public
 */
export interface MediaOp { type: OpType; items: OpItem[]; dest?: string; name?: string }
/** One item whose planned target is already occupied.
 * @public
 */
export interface Conflict { item: OpItem; targetPath: string; type: 'file-exists' | 'folder-exists' }
/** `previewRelocation`'s output: the op's impact plus any target-occupancy conflicts.
 * @public
 */
 ...
 * @public
 */
export function planItem(db: MediaDb, op: MediaOp, item: OpItem): { affected: AffectedSet; plan: ItemPlan } {
...
/** One item's outcome from `executeRelocation`.
 * @public
 */
...
function relocateFolderRows(db: MediaDb, plan: ItemPlan): void {
  db.transaction(() => {
...
function createCopiedFolderRows(db: MediaDb, plan: ItemPlan): void {
...
async function executeItem(
  db: MediaDb, driver: StorageDriver, op: MediaOp, item: OpItem, plan: ItemPlan,
...
export async function executeRelocation(
  db: MediaDb, driver: StorageDriver, op: MediaOp,
...
async function applyStrategy(
  db: MediaDb, driver: StorageDriver, op: MediaOp, item: OpItem,
...
function keyIsFree(db: MediaDb, key: string): boolean {
...
function folderRootIsFree(db: MediaDb, path: string): boolean {
  const cols = getTableColumns(media)
...
async function deleteOccupant(
  db: MediaDb, driver: StorageDriver, item: OpItem, plan: ItemPlan, relocatedIds: Set<number>,
): Promise<boolean> {
  ...
  if (item.type === 'file') {
    ...
    if (row) {
      ...
      await deleteAffected(db, driver, [{ type: 'file', id: row.id }])
    }
    return true
  }
  ...
  await deleteAffected(db, driver, [{ type: 'folder', path: plan.toRoot }])
  return true
}

/** Build a plan whose target is free, for the rename strategy (file → freed filename; folder → freed root). */
function freeRenamePlan(db: MediaDb, item: OpItem, plan: ItemPlan): ItemPlan {
  if (item.type === 'file') {
...
function freeFolderRoot(db: MediaDb, toRoot: string): string {
