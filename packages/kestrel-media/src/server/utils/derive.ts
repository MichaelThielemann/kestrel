...
/** The resolved image-derivation policy (variant widths/formats/quality) a caller threads through `deriveImage`.
 * @public
 */
...
/** Mime types that get a derivative ladder generated.
 * @public
 */
export const RASTER = new Set(['image/png', 'image/jpeg', 'image/webp', 'image/avif'])

/** `deriveImage`'s output: the original's real dimensions, a thumbhash placeholder, and the generated variants.
 * @public
 */
...
/** Generates the thumbhash placeholder and the active variant ladder (per `policy`) for an uploaded raster image.
 * @public
 */
