...
import { setAuthSession, clearAuthSession, getAuthSession, refreshAuthSession } from '../../../src/server/utils/session-cookie.js'
import { sessionSettings } from '../../../src/server/utils/session.js'
import { _resetSessionEpochCache } from '../../../src/server/utils/session-epoch.js'
...
describe('logout wiring: bumping the epoch kills an already-issued cookie', () => {
  it('a cookie signed before bumpSessionEpoch fails getAuthSession after it', async () => {
    ...
    _resetSessionEpochCache()
    const { bumpSessionEpoch } = await import('../../../src/server/utils/session-epoch.js')
