...
describe('login-throttle', () => {
  it('assertBodyLimit throws 413 over the limit, 411 when length is absent/non-numeric, passes otherwise', () => {
    expect(() => assertBodyLimit(String(MAX_LOGIN_BODY + 1), MAX_LOGIN_BODY)).toThrowError(/too large/i)
    expect(() => assertBodyLimit('100', MAX_LOGIN_BODY)).not.toThrow()
    // Absent length == chunked transfer == unbounded buffering: refuse rather than read.
    expect(() => assertBodyLimit(undefined, MAX_LOGIN_BODY)).toThrowError(/length required/i)
    expect(() => assertBodyLimit('not-a-number', MAX_LOGIN_BODY)).toThrowError(/length required/i)
  })
