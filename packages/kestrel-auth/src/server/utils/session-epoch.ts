...
function epochPath(): string | null {
  ...
  // Resolve the DB the same way useDb() does — the boot-time-resolved config (00.config.ts's seam), so
  // this always agrees with wherever the real db actually lives. Landing anywhere else would put
  // revocation state outside the volume the operator persists, so a restart would resurrect revoked
  // sessions.
  const dbPath = getResolvedKestrelConfig().dbPath
