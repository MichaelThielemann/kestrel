...
/** Login's own body cap (used with the shared `assertBodyLimit`): the only legitimate login body is a tiny
 *  JSON object with a known length.
 * @public
 */
export const MAX_LOGIN_BODY = 4096
/** How many `verifyPassword` scrypt hashes may run concurrently — the slot `acquireHashSlot`/
 *  `releaseHashSlot` guard.
 * @public
 */
export const MAX_INFLIGHT_HASHES = 4

/** Failed attempts within `LOGIN_WINDOW_MS` before `assertNotLockedOut` throws.
 * @public
 */
...
/** Throws 429 once `ip` has `MAX_LOGIN_FAILS` recent failures within `LOGIN_WINDOW_MS`.
 * @public
 */
...
/** Records one failed attempt for `ip`, evicting the oldest-tracked IPs once `MAX_TRACKED_IPS` is exceeded.
 * @public
 */
...
/** Clears `ip`'s recorded failures (a successful login). */
/** @public */
...
/** Reserves one of `MAX_INFLIGHT_HASHES` concurrent scrypt slots, throwing 503 once saturated.
 * @public
 */
...
/** Releases a slot reserved by `acquireHashSlot`.
 * @public
 */
