...
/** A content row as `classifyWrite`'s `before`/`after` take it — `null` for a side that does not exist
...
/** Reduce a before/after row pair (either may be null) to a `WriteClassification`. Pure (routes via the
 *  shared `pageRowHref`).
 * @public
 */
...
/**
 * What the write listener enqueues for a content write. `publishOnSave` (`output.publishOnSave`) restores
 * the model where a save is also a publish: with it on, a write plans a full `planInvalidation`. Off (the
 * default), only removals pass — see `planSaveInvalidation`.
 * @public
 */
