...
import { classifyWrite, planInvalidation, planSaveInvalidation, planWrite, routesToPrune } from '../../../../src/server/utils/publish/invalidation.js'
...
describe('classifyWrite — the breadcrumb (page-path) edge', () => {
  ...
  it('names the record’s own path, so its descendants re-render', () => {
    ...
    const inv = planInvalidation(classifyWrite(page, seoPub(), seoPub(), 'en'))
