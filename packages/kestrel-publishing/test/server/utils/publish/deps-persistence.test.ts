...
import { createSqlitePersistence, type DepsPersistenceDb } from '../../../../src/server/utils/publish/deps-persistence.js'
import { DepsStore } from '../../../../src/server/utils/publish/deps.js'

// Exercises `createSqlitePersistence` against a raw, unwrapped drizzle instance (unchecked — no
// `<Module>Db` ownership adapter, on purpose: it's the pure-logic suite, not the ownership suite).
// `DepsPersistenceDb` is branded so a raw handle no longer satisfies it structurally — `freshDb()`'s cast
// is the one place that deliberately steps around that, mirroring `record-ref-index.test.ts`'s own
// `asContentDb` helper.
function freshDb(): DepsPersistenceDb {
  const sqlite = new Database(':memory:')
  sqlite.exec('CREATE TABLE publish_deps (route TEXT PRIMARY KEY NOT NULL, tags TEXT NOT NULL)')
  return drizzle(sqlite) as unknown as DepsPersistenceDb
}
...
describe('createSqlitePersistence (durable publish_deps backing)', () => {
  ...
  it('degrades gracefully when the publish_deps table is absent (a not-yet-migrated deploy)', () => {
    const bare = drizzle(new Database(':memory:')) as unknown as DepsPersistenceDb // no publish_deps table
