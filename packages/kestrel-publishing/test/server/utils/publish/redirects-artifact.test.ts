...
describe('the collection contract', () => {
  it('names the collection and its repeater field once, for every consumer of the row', async () => {
    const { REDIRECTS_COLLECTION, REDIRECTS_FIELD } = await import('../../../../src/server/utils/publish/redirects-artifact.js')
    const redirects = (await import('../../../../src/server/collections/redirects.js')).default
