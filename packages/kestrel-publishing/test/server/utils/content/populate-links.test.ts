...
import { buildLinkFieldPopulators } from '../../../../src/server/utils/content/populate-links.js'
import { buildFieldTreePopulator, defineBlock, registerBlock, clearBlocks  } from '@michaelthielemann/kestrel-fields'
import { defineCollection, pageRowHref, withReadCaptureSync, withResolveScope } from '@michaelthielemann/kestrel-core'
...
describe('pageRowHref — localized route of a page-like row (unconditional; only missing/path-less rows are unresolvable)', () => {
  it('returns the localized path for a target (primary unprefixed, other-locale prefixed)', () => {
    expect(pageRowHref({ path: '/about', locale: 'en', status: 'published' } as Record<string, unknown>, 'en')).toBe('/about')
    expect(pageRowHref({ path: '/ueber-uns', locale: 'de', status: 'published' } as Record<string, unknown>, 'en')).toBe('/de/ueber-uns')
  })
  it('builds the route from a DRAFT row too — the status gate sits above it, in resolveInternalHref', () => {
    expect(pageRowHref({ path: '/about', locale: 'en', status: 'draft' } as Record<string, unknown>, 'en')).toBe('/about')
    expect(pageRowHref({ path: '/ueber-uns', locale: 'de', status: 'draft' } as Record<string, unknown>, 'en')).toBe('/de/ueber-uns')
  })
  it('returns null only for a missing row or a row with no path (→ "#")', () => {
    ...
    expect(pageRowHref({ path: null, status: 'published' } as Record<string, unknown>, 'en')).toBeNull()
    expect(pageRowHref({ status: 'draft' } as Record<string, unknown>, 'en')).toBeNull()
  })
  ...
  it('falls back to the primary locale when the target row has no locale', () => {
    expect(pageRowHref({ path: '/x', status: 'published' } as Record<string, unknown>, 'en')).toBe('/x')
    expect(pageRowHref({ path: '/x', locale: null, status: 'draft' } as Record<string, unknown>, 'en')).toBe('/x')
  })
})
