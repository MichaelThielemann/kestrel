...
describe('the barrel seeds field types before its own buildCollection() calls', () => {
  ...
  // (ADR-0029) — so whatever registers `@michaelthielemann/kestrel-fields`' built-in field types has to be the barrel's
  ...
  // bundler respecting `"sideEffects": false` may tree-shake a bare `import '@michaelthielemann/kestrel-fields'` (it names
  ...
  it('imports @michaelthielemann/kestrel-fields as a used binding, not a bare side-effect import, as the very first import', () => {
    const firstImport = /^import\b.*$/m.exec(indexTs)?.[0]
    expect(firstImport).toMatch(/^import \{ fieldTypes \} from '@michaelthielemann\/kestrel-fields'$/)
  })
...
describe('the barrel actually seeds the registry before any consumer-visible collection builds', () => {
  it('has the field registry seeded immediately after importing only ensureSnapshotTriggers from a fresh module graph', async () => {
    ...
    const { fieldTypes } = await import('@michaelthielemann/kestrel-fields')
