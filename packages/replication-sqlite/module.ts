...
export default defineModule({
  ...
  describe: () => ({
    ...
    prepareRestore: { summary: "Rebuild the database at a point in time next to the live file; applied on next start", reads: [], writes: ["result"], input: { type: "object", properties: { generation: { type: "string" }, at: { type: ["number", "string"], description: "milliseconds or ISO-8601; default latest" } } }, output: { type: "object", properties: { generation: { type: "string" }, at: { type: "number" }, file: { type: "string" }, restartRequired: { type: "boolean", enum: [true] } }, required: ["generation", "at", "file", "restartRequired"] }, errors: { 400: 'invalid "at" value', 404: "no snapshot before the requested point" } },
  }),
});
