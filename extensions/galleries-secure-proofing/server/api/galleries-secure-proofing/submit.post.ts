...
// Public PROOFING back-channel. Anonymous-writable via the registered access grant (resource
// `galleries-secure-proofing`), but hardened in depth: the core guard's same-site CSRF check runs FIRST, then
// this route rate-limits per IP, caps the buffered payload size, and — for a NEW row — enforces a per-gallery
// cap, a GLOBAL row backstop (the slug is attacker-controlled, so a rotating slug must not grow storage
// without limit), and a per-IP-per-slug NEW-identity budget (so one IP can't fill a real gallery's cap to
// lock out legitimate reviewers). Every bound REJECTS the write; nothing on this anonymous path ever deletes
// a stored row. It stores ONLY the customer's client-encrypted ciphertext — the server never sees the marks
// (zero-knowledge). Upserts one row per (gallerySlug, customerId) into the `galleryProofing` collection.
// `parseSubmission` / `exceedsGalleryQuota` / `exceedsGlobalQuota` / `newCustomerRateKey` / `rateLimit` /
// `readCappedBody` (this layer), `clientIp` + `throttleKey` (auth), `useDb` / `getCollection` / `create` /
// `update`, and h3 helpers are auto-imported.
...
export default defineEventHandler(async (event) => {
  ...
  const raw = await readCappedBody(event.node.req, MAX_BODY_BYTES)
  if (raw === null) {
  ...
  if (!existing) {
    ...
    // fill a real gallery's per-gallery cap and lock out every genuinely-new reviewer. Only new rows consume it.
  ...
  const row = (existing ? update(db, c, existing.id, body) : create(db, c, body)) as { id: number }
