...
// media-cleanup plugin. `registerWriteListener` / `useStorageDriver` / `planGalleryDeletion` /
// `referencedGalleryIds` (this layer) + `useDb` / `allCollections` are auto-imported.
export default defineNitroPlugin(() => {
  registerWriteListener((event) => {
    ...
    // verbatim multilingual copy) — that would delete the other record's blobs. Fail-safe: on any scan error,
    // treat every id as in-use → skip deletion (an orphaned namespace is recoverable; deleting live data isn't).
    let inUse: Set<string>
    try { inUse = referencedGalleryIds(useDb(), allCollections()) }
    catch (error) { console.error('[kestrel] gallery ownership scan failed; skipping cleanup:', error); return }
