...
import { createLocalDriver } from '../../../../../layers/core/server/utils/storage.local'

// The handler is a Nitro route: everything it calls (defineEventHandler / requireAdmin / readBody /
// useStorageDriver / the gallery server utils) is auto-imported at runtime. Stub those globals, then import
// the module and drive the exported handler directly — the same shape a Nitro request would.
...
const event = (body: unknown) => ({ body, headers: { 'content-length': String(JSON.stringify(body).length) } })

beforeAll(async () => {
  const g = globalThis as Record<string, unknown>
  g.defineEventHandler = (h: unknown) => h
  g.requireAdmin = () => {}
  g.mediaRuntimeConfig = () => ({ maxUploadBytes: 10_000_000 })
  g.getRequestHeader = (e: { headers: Record<string, string> }, name: string) => e.headers[name]
  g.readBody = async (e: { body: unknown }) => e.body
  g.useStorageDriver = () => driver
  g.createError = createError
  g.galleryNamespace = galleryNamespace
  g.orphanBlobKeys = orphanBlobKeys
  g.stalePruneKeys = stalePruneKeys
  handler = ((await import('./tree.put')).default) as unknown as (event: unknown) => Promise<unknown>
})
...
describe('PUT /tree — index write concurrency guard', () => {
  it('accepts a write whose seq advances the stored one', async () => {
    putStored({ v: 1, seq: 4, files: [], folders: [] })
    await handler(event({ galleryId: GID, index: { v: 1, seq: 5, files: [fileEntry(A)], folders: [] } }))
  ...
  it('rejects (409) a stale write whose seq does not advance, leaving the stored index untouched', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 4, files: [], folders: [] } })))
      .rejects.toMatchObject({ statusCode: 409 })
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 5, files: [], folders: [] } })))
      .rejects.toMatchObject({ statusCode: 409 })
  ...
  it('rejects (409) a write with no seq once the stored index carries one', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, files: [], folders: [] } })))
      .rejects.toMatchObject({ statusCode: 409 })
    expect(storedIndex().files).toHaveLength(1)
  })
  ...
  it('a rejected write never reaches the orphan reconcile (the ciphertext survives)', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 1, files: [], folders: [] } })))
      .rejects.toMatchObject({ statusCode: 409 })
  ...
  it('legacy: a stored index without seq is still writable (no guard to enforce)', async () => {
    ...
    await handler(event({ galleryId: GID, index: { v: 1, seq: 1, files: [fileEntry(A)], folders: [] } }))
    expect(storedIndex().seq).toBe(1)
  })

  it('a gallery with no stored index yet accepts the first write', async () => {
    await handler(event({ galleryId: GID, index: { v: 1, seq: 1, files: [], folders: [] } }))
    expect(storedIndex().seq).toBe(1)
  })
  ...
  it('refuses the write when the stored index exists but the read FAILS (not the same as "no index")', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 2, files: [fileEntry(A)], folders: [] } })))
  ...
  it('refuses the write when the presence probe itself fails (doubt is not absence)', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 2, files: [], folders: [] } })))
      .rejects.toMatchObject({ statusCode: 503 })
    expect(storedIndex().seq).toBe(9)
  })
...
describe('PUT /tree — a damaged stored index', () => {
  ...
  it('an explicit repair write replaces the damaged index', async () => {
    damaged(Buffer.from(GARBLED))
    await handler(event({ galleryId: GID, index: { v: 1, seq: 1, files: [], folders: [] }, repair: true }))
    expect(storedIndex().seq).toBe(1)
  })
  ...
  it('repair is inert against a READABLE stored index — it never bypasses the seq guard', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 4, files: [], folders: [] }, repair: true })))
      .rejects.toMatchObject({ statusCode: 409 })
    expect(storedIndex().seq).toBe(5)
    expect(storedIndex().files).toHaveLength(1)
  })
  ...
  it('repair is inert when the read itself FAILED — doubt still refuses', async () => {
    ...
    await expect(handler(event({ galleryId: GID, index: { v: 1, seq: 2, files: [], folders: [] }, repair: true })))
      .rejects.toMatchObject({ statusCode: 503 })
    expect(storedIndex().seq).toBe(9)
  })
})

describe('PUT /tree — orphan reconcile', () => {
  ...
  it('prunes a stale unreferenced blob when the index does not shrink', async () => {
    ...
    await handler(event({ galleryId: GID, index: { v: 1, seq: 2, files: [fileEntry(A)], folders: [] } }))
