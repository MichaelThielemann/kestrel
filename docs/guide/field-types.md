...
Collections are defined with `defineCollection`; see [collections.md](./collections.md) for the
...
Need something else? See [custom field types](./custom-field-types.md).
...
| Flag | What it does |
|---|---|
| `required` | For a scalar-backed type, makes the column `NOT NULL` and the validator reject empty. **Only when there is no `condition`** — a conditional field stays nullable in the database and is enforced at validation time instead. For the JSON/array-backed types (multi `choice`, multi `media`, a `many` `relation`, `repeater`, `json`) the column is always `NOT NULL` regardless of `required`; there `required` only makes the validator demand a non-empty array/value — see each type's section. |
...
| `condition` | Shows or hides the field based on another field's value. Evaluated in the editor *and* re-checked on the server for conditionally-required fields. The full condition grammar is in [field-layout.md](./field-layout.md) § "Conditional fields". |
...
| `populate` | Overrides the type's populator for this one field — see [custom field types](./custom-field-types.md) and [population internals](../internals/populate.md). |
...
**Column** `text`. The value is always run through `slugify`, whether typed or derived. Uniqueness on a
`slug` field marked `unique: true` is pre-checked against the database on every write, so a collision is
reported on the field rather than as a generic constraint error.
...
unpublished target renders as `#`. See [references.md](./references.md).
...
| `units` | `string[]` | Accepted and ignored — no code reads it |

> **A number field is an integer by default.** The column is `real` only if you pass `integer: false`
> or a `decimals` value. Sending `19.99` to a field you did not opt out of integer mode is rejected by
> the validator, not rounded.
...
No options. **Column** `integer` in boolean mode. The column is nullable, so an untouched optional boolean
stays `null` and the widget shows no option selected — but once true or false is picked, the two-option
button group offers no way back to unset (clear it through the API, or give the field a `default`).
...
```ts
{ type: 'choice'; options: { choices: { label: string | Record<string, string>; value: string }[]; multiple?: boolean; display?: 'select' | 'buttons' | 'checkboxes' } }
```
...
| Option | Type | Enforced by |
|---|---|---|
| `choices` | `{ label, value }[]` | Server — becomes a `z.enum` of the values. `label` accepts the same per-language record as the field `label` flag |
| `multiple` | `boolean` | Server — changes both the column and the validator |
| `display` | `'select' \| 'buttons' \| 'checkboxes'` | **Widget only** — and only `'buttons'` is read |

**Column** `text` for a single choice, a JSON array when `multiple`.

Only `display: 'buttons'` changes the control. Any other value — `'select'`, `'checkboxes'`, or omitted —
is accepted and ignored: the control then follows `multiple` alone, a select when single and a checkbox
group when `multiple`.
...
**Column** a single media field stores a media id in `integer`, named `<key>Id` in the API and the record
and `<key_snake_case>_id` as the SQLite column (`heroImage` → `heroImageId` / `hero_image_id`). With
`multiple: true` it is a JSON array and the key is unchanged.

On read the resolved media records are attached under `$media.<field>`; a dangling id resolves to
`null` rather than failing the read. This only happens when the read asks for it — population runs at
`?depth` ≥ 1, and `?depth` defaults to `0`, so a plain read returns raw ids. See [media.md](./media.md)
and [querying.md](./querying.md) § "`?depth` and population".
...
**Column** as with `media`: a single relation is `integer` named `<key>Id` in the API and the record and
`<key_snake_case>_id` as the SQLite column, a `many` relation is a JSON array under the plain key.

Resolved records are attached under `$<field>`, only when the read asks for it (`?depth` ≥ 1; the default
`0` returns raw ids) — and a public read never expands a relation into a non-public collection. See
[querying.md](./querying.md) § "`?depth` and population". The related read is **published-only on every
read**, not just a public one — a draft target resolves to `null` for a single relation, or is dropped
from the array for a `many` relation. Other errors propagate. Nested population is depth-limited; see
[population internals](../internals/populate.md).
...
| `fieldLayout` | `FieldLayoutDSL` | Arranges the sub-fields inside a row, in the same DSL as a collection's own layout — see [field-layout.md](./field-layout.md) § "Repeater layouts". Only affects presentation, but is still validated at `defineCollection()` |
...
resolved per row, stored under the bare field key (no `Id` suffix — that column-naming rule applies only
to a top-level field). A conditionally-required sub-field is re-checked per row.
...
No options. **Column** `text` in JSON mode, always `NOT NULL` — unconditionally, it does not follow
`required` — defaulting to the field's declared `default`, or `{}` when none is given.

Separately, the validator maps an explicit `null` to `{}` on write, regardless of the declared default —
so an optional `json` field always stores `{}` for a PATCH `null`, even one whose `default` is something
else. A required `json` field rejects `null` and `undefined` but accepts any other JSON value, including
`{}`.
...
Need a type that isn't in the table above? Register a server descriptor with `defineFieldType`, then a
widget with `registerFieldComponent`. The full walkthrough — including the `column`/`validator`
contract, `registerFieldEmpty`, and the per-instance `populate` override — is in
...
`required: true` combined with a `condition` never makes the column `NOT NULL`; the requirement is
enforced at validation time only. See [field-layout.md](./field-layout.md) § "Conditional fields" for the
condition grammar and how `required` is re-enforced while the field is visible.

## See also

- [collections.md](./collections.md) — where fields are assembled into a collection definition.
- [field-layout.md](./field-layout.md) — the `fieldLayout` DSL and the `condition` grammar both referenced above.
- [custom-field-types.md](./custom-field-types.md) — registering a new type, its widget, and per-instance `populate`.
- [references.md](./references.md) — status-gated link resolution for `link` and `richtext`, and the dead-reference report.
- [querying.md](./querying.md) — `?depth` and population: when `media`/`relation` values actually resolve.
- [../internals/populate.md](../internals/populate.md) — how the field-tree walker calls each type's populator.
