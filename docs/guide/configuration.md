...
The single config surface: the `kestrel: { … }` key in `nuxt.config.ts`, every `KESTREL_*` env var and its precedence over it, and the env-only settings that never live there.

## nuxt.config.ts: the kestrel key

All **non-auth** configuration lives in one place: the **`kestrel: { … }`** key in **`nuxt.config.ts`**
(see [Getting started](./getting-started.md)). Nothing loads a project-root `kestrel.config.ts`
automatically — that filename resolves inside Kestrel's own package, not your project. The resolved
values are consumed by:

- the **`kestrel` module** → merges them into Nuxt's `runtimeConfig`,
- the **server at runtime** (db / locale / site-url utils), reading that `runtimeConfig`,
- the **client**, for the values the module exposes under `runtimeConfig.public`.

For a large config, extract it into its own file and import it — `import kestrel from './kestrel.config'`
in `nuxt.config.ts`, then `kestrel: { ...kestrel }` — but the file itself is a plain object export with no
special loading; nothing reads that filename by convention.

The block below is illustrative, not a copy-pasteable default — most fields shown here already equal the
built-in default; where a value differs it is called out.
...
export default {
  ...
  siteUrl: 'https://example.com',     // sitemap <loc> + robots Sitemap: (no built-in default)
  ...
  locales: ['en', 'de'],              // website/content locales (this IS the built-in default)
  primaryLocale: 'en',                // default: the first entry of the resolved locale list;
                                       // a value not present in `locales` is ignored and falls back to it
  prefixPrimaryLocale: false,         // true → prefix the primary locale too (/en/about); root → /<primary>
  media: {
    uploadDir: '.data/uploads',       // written to AND served at /uploads (single dir, can't drift)
    baseUrl: '/uploads',
    ...
    image: {                          // responsive-image derivation (usage-driven; see media.md)
  ...
  output: {                           // where the published static site is written (separate from media)
    ...
    dir: '.data/published',           // local dir for the published HTML + synced _nuxt
    ...
    auto: true,                       // run the publisher in-process (publishing is an explicit action)
                                       // — with driver: 's3', auto: true also makes `nuxt generate` skip
                                       // the S3 deploy entirely; set false for the build-time deploy model,
                                       // see deploying.md
    publishOnSave: false,             // true → every save republishes again
    ...
    s3: {                             // used only when driver: 's3'
      ...
      region: 'eu-central-1',
      endpoint: '',                   // S3-compatible origin (R2/MinIO); omit for AWS S3
      prefix: 'site',                 // required: the deploy reconciles (deletes) everything under it
    },
  },
  ...
  aiDisclosure: {                     // EU AI Act Art. 50 disclosure on media assets — see ai-disclosure.md
...
Per setting: **matching `KESTREL_*` env var → `kestrel: {}` value → built-in default.** The committed
...
*When the environment is read* below; a test suite might set `KESTREL_DB` / `KESTREL_MEDIA_LOCAL_DIR` / `KESTREL_SITE_URL` per run). `dev` and
`build` set no `KESTREL_*` on their own, so the config wins and they never drift onto different
DBs/paths. (Auth/session settings are the exception — they are env-**only**; they are never read from
this file.)
...
`delivery`→`KESTREL_DELIVERY`, `deliveryExempt`→`KESTREL_DELIVERY_EXEMPT`.

`delivery` (`'static' | 'live'`, default `'static'`, anything else falls back to `'static'`) picks which
`DeliveryPort` serves traffic, and `deliveryExempt` (comma-separated path prefixes) carves out routes the
live catch-all skips; see [Publishing](./publishing.md) for both. `revisions: { keep, maxAgeDays }` is
resolved config-only, with no env override — but the module does not yet forward it into `runtimeConfig`,
so a `kestrel: { revisions: {...} }` value does not currently reach the server; retention stays at the
`'all'` default until that's wired up. See [Revisions](./revisions.md).

See [AI disclosure](./ai-disclosure.md) for what `aiDisclosure.enabled` gates and the `aiSourceType` /
`aiNote` fields themselves.
...
[Deploying](./deploying.md) for the full output-target walk-through.

The same resolved values feed the client and the server at runtime, so they can never disagree. Schema
changes are applied separately, by the `db:migrate` task, not by resolving this config — see
[Schema changes and migrations](./schema-lifecycle.md). See [Data model](../internals/data-model.md) for
how resolution is wired in.
...
`process.env` directly. So a `KESTREL_*` var must be present in the environment that **builds** the app;
setting one in front of an already-built server does nothing:
...
the wrong database. The same holds for media/output S3 credentials: the storage driver sees the baked
...
| output target | `KESTREL_OUTPUT_DIR` / `_DRIVER` / `_AUTO` | `NUXT_KESTREL_OUTPUT_DIR` / `NUXT_KESTREL_OUTPUT_DRIVER` / `NUXT_KESTREL_OUTPUT_AUTO` |
...
Every value the module writes into runtimeConfig follows the same rule (`runtimeConfig.media.local.dir` →
`NUXT_MEDIA_LOCAL_DIR`). Note the two spellings differ on purpose: `KESTREL_*` names the *config* key,
`NUXT_*` names the *runtimeConfig* key the module wrote it to. `nuxt generate` output has no server at all,
so there only the build environment exists. The auth/session and access-gate vars are the exception to all
of this — they are read from `process.env` per request and work as-is on a prebuilt server; see
[IP allow-list](#ip-allow-list--optional) below.
...
- `KESTREL_ADMIN_PASSWORD_HASH` — scrypt hash of the single admin password (also folded into the key).
  When unset, `POST /api/login` answers **503** ("admin login is not configured") rather than a
...
- `KESTREL_SESSION_MAX_AGE` — session **idle window** in seconds (default 604800). Activity slides the
  ...
  session immediately regardless of this setting, use `POST /api/logout` (bumps the revocation
  epoch below).
...
- `KESTREL_TRUST_PROXY` — trusted reverse-proxy depth for deriving the client IP. Shared by the
  login-throttle **and** the IP allow-list below (both derive the client IP the same way). Default off →
  the socket peer is used and `X-Forwarded-For` is ignored (it is attacker-spoofable without a proxy).
  Set `true`/`1` for a single trusted proxy, or an integer N for N chained proxies; the Nth-from-right XFF
  hop is then trusted. Enable this only when a proxy actually fronts the app. Like the vars above, it is
  read from `process.env` per request, not baked at build — it can be set on a prebuilt server and has no
  `NUXT_*` counterpart.

Generate a password hash with `pnpm kestrel hash-password <password>` (see [Getting started](./getting-started.md)).

## IP allow-list — optional

A stage-level gate that covers every route this app defines — admin UI, public preview, and API — not
just `/api/*`. Off by default; typically used to lock a non-public DEV/EDIT stage down to an office/VPN
range. Like the auth/session vars above, `KESTREL_IP_ALLOWLIST` / `_MODE` are read from `process.env` at
runtime, not baked into `runtimeConfig` at build — they can be set (or changed) on a prebuilt server and
have no `NUXT_*` equivalent.

It does **not** cover the assets Nitro serves ahead of the middleware stack: `/_nuxt/**`, and `/uploads`
when `media.driver: 'local'`. Nitro serves its own static handler before any middleware runs, so those
...
- `KESTREL_IP_ALLOWLIST` — the allow-list, IPv4/CIDR only (`1.2.3.4`, `10.0.0.0/8`). Newline/comma/
  ...
  nginx `allow` block can be pasted in verbatim. Invalid tokens (including IPv6 CIDRs) are dropped with a
  warning rather than failing the whole list; a list that parses down to zero valid CIDRs blocks **all**
  traffic in `enforce` mode. An IPv4-mapped IPv6 peer address (`::ffff:203.0.113.9`, what a dual-stack
  listener reports for an IPv4 client) is unwrapped and matched as its IPv4 form; a genuine IPv6 client
  never matches and is blocked. Unset or blank ⇒ the gate is `off` regardless of the mode below.
- `KESTREL_IP_ALLOWLIST_MODE` — `enforce` (default the moment a list is set; a blocked request gets
  **403**), `log` (never blocks; logs what *would* be blocked, to calibrate `KESTREL_TRUST_PROXY` before
  enforcing), or `off`.
...
(build/dev start), unconditionally, and copies them into `runtimeConfig.media.s3`, which is what the driver
later reads. Two consequences:
...
- They must be in the **build** environment, and they then sit in the build artifact. Keep them out of a
  ...
  process instead — see *When the environment is read* above.
...
the normal `KESTREL_S3_* → config → default` precedence above. See [Media](./media.md) for the full
driver walk-through.
...
So to point uploads and the published site at *different* accounts, set the `KESTREL_OUTPUT_S3_*` trio.
See [Deploying](./deploying.md) for the deploy walk-through.

## State & backups

All mutable state lives under three paths (all gitignored), not in the code or the build artifact:
...
- [Getting started](./getting-started.md) — scaffolding a project and generating the auth env vars.
- [Media](./media.md) — the storage drivers, image derivation, and AI-disclosure fields these settings control.
- [Deploying](./deploying.md) — the output target, S3 deploy, and dry-run walk-through.
- [Data model](../internals/data-model.md) — how config resolution feeds the DB and runtime.
