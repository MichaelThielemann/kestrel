...
/**
 * Every `@kestrel/*` workspace package with a `src/` dir, computed from `packages/*` on disk —
 * `create-kestrel` (no `src/`, no `@kestrel/*` name) is excluded without a hand list. Mirrors
...
function kestrelPackageNames(): string[] {
  ...
  return readdirSync(base, { withFileTypes: true })
    ...
    .filter((name) => name.startsWith('@kestrel/'))
}

/**
 * `test:packages`/`typecheck:packages` (package.json) and the standalone-packages step in
 * `scripts/typecheck.mjs` all glob `pnpm --filter "@kestrel/*" -r <script>` — no hand list, nothing for
 ...
 */
describe('every @kestrel/* package appears in the hand-written root script chains', () => {
  ...
  for (const scriptName of ['api:check', 'api:update', 'pkg:lint']) {
    it(`${scriptName} mentions every @kestrel/* package`, () => {
