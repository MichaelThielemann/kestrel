...
# `pnpm publish`, not `npm publish`: several packages (the engine root, and every @michaelthielemann/kestrel-* package) carry
# a real `workspace:*` dependency on another workspace package — only pnpm's own publish/pack rewrites that
# to the real resolved version before upload (confirmed via `pnpm pack`: `"@michaelthielemann/kestrel-core": "workspace:*"`
# becomes `"@michaelthielemann/kestrel-core": "0.1.0"` in the packed manifest). `npm publish` does not understand the
